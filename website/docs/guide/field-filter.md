...
如果你还需要控制“哪些记录能进入结果集”，先看 [行级权限](/guide/row-level)。字段过滤负责的是记录已经进入之后的字段收口，不负责替代行级范围判断。
...
想了解数据和接口权限在运行时依赖什么存储，下一篇看 [存储适配器](/guide/adapters)。

## 下一步看什么

- 想继续看接入前的确认项：看 [接入检查清单](/guide/integration-checklist)
- 想看真实字段过滤落地方式：看 [字段权限示例](/examples/field-permission)
- 想继续按接入顺序阅读：看 [接入阅读顺序](/guide/implementation-reading-order)