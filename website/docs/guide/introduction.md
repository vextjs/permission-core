...
- `<METHOD>:<path>` 与 `db:<collection>[:<field>]` 两类资源
...
2. [常见问题](/guide/faq)
3. [资源路径模型](/guide/resource-paths)
4. [角色与规则](/guide/roles-and-rules)
5. [权限鉴权](/guide/check-permission)
6. [接入检查清单](/guide/integration-checklist)

当你已经准备开始写接入代码，再继续看 [接入阅读顺序](/guide/implementation-reading-order)。

## 主路径之后按场景补读

### 你在做权限模型或后台规则维护

1. [角色与规则](/guide/roles-and-rules)
2. [RoleManager](/api/role-manager)
3. [UserRoleManager](/api/user-roles)
4. [管理后台接入](/guide/site-preview-release)

### 你在做框架接入或 Service 分层

1. [框架接入](/guide/framework-integration)
2. [Express 接入](/examples/express)
3. [vext 接入](/examples/vext)
4. [PermissionCore](/api/permission-core)

### 你在做数据权限落地

1. [行级权限](/guide/row-level)
2. [字段过滤](/guide/field-filter)
3. [字段权限示例](/examples/field-permission)
4. [PermissionCore](/api/permission-core)