...
如果你已经大致看过 [快速开始](/guide/quick-start)，但还不确定该选哪条路径、该把规则放在哪里，先看这页通常比直接跳到 API 参考更有效。
...
如果你是做运营后台或权限配置后台，还可以继续看 [管理后台接入](/guide/site-preview-release)。
...
如果你想看完整边界说明，包括“为什么默认推荐 MonSQLize”以及“为什么这不等于 MongoDB 专属权限模型”，直接看 [存储适配器](/guide/adapters)。
...
2. 再看 [资源路径模型](/guide/resource-paths)
3. 然后看 [角色与规则](/guide/roles-and-rules)
4. 最后按需要进入 [框架接入](/guide/framework-integration) 或 [PermissionCore](/api/permission-core)

## 下一步看什么

- 如果你已经准备开始真正接入：看 [接入检查清单](/guide/integration-checklist)
- 如果你还是不确定应该怎么选路径：回看 [快速开始](/guide/quick-start)
- 如果你准备开始写接口层接入：看 [框架接入](/guide/framework-integration)