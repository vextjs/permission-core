...
- 想先理解资源格式为什么保持不变：看 [资源路径模型](/guide/resource-paths)
- 想看集合、行、字段三层在运行时怎么串起来：看 [权限鉴权](/guide/check-permission)
- 想直接看 `getRowScope()` / `canRow()` / `assertRow()` / `filterRows()` 串起来的真实代码：看 [行级权限示例](/examples/row-level)
- 想看字段收口怎么落地：看 [字段过滤](/guide/field-filter)