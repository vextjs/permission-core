...
如果你还没完成最小接入，先回看 [快速开始](/guide/quick-start) 和 [接入检查清单](/guide/integration-checklist)。
...
- 规则变化后谁负责调用 `invalidateAll()`
- 用户角色变化后谁负责调用 `invalidate(userId)`
...
### 误区二：只在管理后台写规则，不做失效策略

如果规则更新后没有配套执行 `invalidateAll()` 或针对用户执行 `invalidate(userId)`，线上看到的就会是“后台改了，前台没生效”。
...
如果你还没确认当前依赖和运行时边界，继续看 [兼容性矩阵](/guide/compatibility-matrix)。