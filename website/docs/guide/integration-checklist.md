...
如果这里还不确定，先回看 [快速开始](/guide/quick-start) 和 [常见问题](/guide/faq)。
...
- [ ] 如果准备上生产，已确认当前运行时与依赖版本满足 [兼容性矩阵](/guide/compatibility-matrix)
...
如果这一块还不稳，建议直接对照 [框架接入](/guide/framework-integration) 和 [Express 接入](/examples/express)。
...
如果这一块还不稳，建议直接对照 [字段过滤](/guide/field-filter) 和 [字段权限示例](/examples/field-permission)。
...
如果这里容易混淆，先回看 [角色与规则](/guide/roles-and-rules) 和 [RoleManager](/api/role-manager)。
...
如果这里还有疑问，先回看 [权限鉴权](/guide/check-permission) 和 [PermissionCore](/api/permission-core)。
...
如果这里还不清楚，建议继续看 [权限缓存](/guide/cache)。
...
如果你已经准备上线，继续看 [生产部署与监控](/guide/production-deployment)。
...
2. 再看 [常见问题](/guide/faq)
3. 然后按需要进入 [框架接入](/guide/framework-integration) 或 [PermissionCore](/api/permission-core)
4. 真正写接入代码前，再回来看这份清单