...
当前仓库已经落地上述核心运行时，并已通过 `typecheck`、59 项测试、构建产物生成，以及语句 / 分支 / 函数 / 行覆盖率 `100%` 验证。
...
跑通之后，再回看 [快速开始](/guide/quick-start)、[资源路径模型](/guide/resource-paths) 和 [PermissionCore API](/api/permission-core)，理解会更直接。
