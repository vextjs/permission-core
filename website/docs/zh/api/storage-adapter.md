...
### 不要把 `setRules()` 暴露成业务批量接口

`StorageAdapter` 是低层持久化契约。`setRules(roleId, rules)` 只表示“覆盖写入某个角色当前存储的规则快照”，不负责角色是否存在、动作和资源是否合法、行级条件是否规范、重复规则怎么合并、allow / deny 冲突怎么解释，也不会替你执行 `RoleManager` 的缓存失效流程。

业务代码通常应该通过 `RoleManager` 保存规则。只有在你有自己的迁移脚本、同步任务或管理服务，并且明确承担校验、冲突处理和缓存失效时，才适合直接写适配器。
