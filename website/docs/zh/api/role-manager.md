...
### 公开写入方法会触发 `invalidateAll()`

这条规则很关键，因为角色规则变化会影响继承链上的下游用户，不只是直接绑定该角色的用户。首版方案优先选择“简单且安全”的全量失效策略。

### 没有通用角色规则批量覆盖 API

`allow()` 和 `deny()` 的 `actions` 参数支持 `string | string[]`。数组形式只表示同一个资源下的多个动作，不是通用的多资源、多规则批量 API。

permission-core v1 没有公开一个“一次覆盖任意角色规则数组”的业务 API。角色规则变化可能影响直接绑定用户、继承链下游用户以及已经缓存的权限集合，所以公开写入仍收口在 `allow()`、`deny()`、`revokeRule()` 和 `clearRules()` 这些显式方法里。它们会处理输入校验、规则去重、deny 优先语义和缓存失效。

如果后台表单一次编辑很多规则，建议在你自己的后端保存服务里接收规则数组，先做校验、去重和差异计算，再调用这些公开方法。不要把 `StorageAdapter.setRules()` 直接当成业务批量接口暴露出去；直接调用它会绕过 `RoleManager` 的校验和缓存失效。
...
这些公开写入方法已经会让相关权限缓存失效。只有你绕过 `RoleManager`、直接写存储适配器，或从外部系统同步规则时，才需要额外规划 `invalidateAll()`。
...
如果你准备把角色和用户绑定做成后台页面，可继续看 [管理后台接入](/zh/guide/site-preview-release)。

如果你接下来要看用户和角色怎么绑定，可继续看 [UserRoleManager](/zh/api/user-roles)。
