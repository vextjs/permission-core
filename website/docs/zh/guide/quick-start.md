...
如果你准备做角色配置页、用户角色页或运营后台，可以继续看 [管理后台接入](/zh/guide/site-preview-release)。
...
- 想理解资源怎么写：看 [资源路径模型](/zh/guide/resource-paths)
- 想理解角色和规则怎么组织：看 [角色与规则](/zh/guide/roles-and-rules)
- 想理解运行时主 API：看 [PermissionCore](/zh/api/permission-core)
- 想看框架接入：看 [框架接入](/zh/guide/framework-integration)
- 想先解开最常见的接入疑问：看 [常见问题](/zh/guide/faq)
- 想真正开始接入前做一次总检查：看 [接入检查清单](/zh/guide/integration-checklist)
