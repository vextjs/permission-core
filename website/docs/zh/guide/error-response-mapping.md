...
如果你已经看过 [错误码](/zh/api/errors)，但还是不确定在中间件、管理 API 或后台接口里该怎么处理，这页更适合继续往下读。
...
这一步的重点不是拿到“实际 URL”，而是拿到稳定的接口资源。只要框架能暴露命中的模板路由，就优先用模板路径；拿不到时再退回普通 `path`。
...
- 想回到错误码本身：看 [错误码](/zh/api/errors)
- 想看接口层真实接法：看 [Express 接入](/zh/examples/express)
- 想看管理能力入口：看 [RoleManager](/zh/api/role-manager) 和 [UserRoleManager](/zh/api/user-roles)
- 想把角色页、用户页和缓存失效串成一条链路：看 [管理后台接入](/zh/guide/site-preview-release)