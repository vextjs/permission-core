...
想看完整运行时分层，可以继续看 [行级权限](/zh/guide/row-level)。
...
- [角色与规则](/zh/guide/roles-and-rules)：看资源如何进入规则模型
- [权限鉴权](/zh/guide/check-permission)：看资源如何进入运行时判断
- [接入阅读顺序](/zh/guide/implementation-reading-order)：看开始接入时应该怎么排阅读和落地顺序

想继续了解规则怎么组合，下一篇看 [角色与规则](/zh/guide/roles-and-rules)。