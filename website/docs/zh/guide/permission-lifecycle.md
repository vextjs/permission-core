...
如果你只是第一次接入，先看[快速开始](/zh/guide/quick-start)和[检查权限](/zh/guide/check-permission)。这页适合下面几种情况：

| 场景 | 是否需要读 |
|---|---|
| 设计应用启动、关闭、健康检查和数据库连接所有权 | 需要 |
| 排查“为什么刚改了权限，用户请求还没生效” | 需要 |
| 排查 revision 冲突、preview 过期、缓存失效和审计证据 | 需要 |
| 只想知道业务里怎么写 `can()` / `assert()` | 先跳过，看检查权限 |
...
| 调用 | 你需要知道什么 | 生命周期作用 |
|---|---|---|
| `pc.init()` | 返回 `PermissionCoreHealth` | 从 new/initializing 进入 ready，失败则启动失败 |
| `pc.scope()` / `forSubject()` | 同步 facade | 绑定管理 scope 或请求 subject，不写数据库 |
| `roles.create/allow`、`userRoles.assign` | 返回管理写入结果，包含修订和审计线索 | 在事务中提交管理状态与审计证据 |
| `subject.can()` | 返回 boolean | 从当前稳定授权状态做请求判定 |
...
运行时在一个 MonSQLize 事务内重新校验修订、来源完整性、层级、容量和幂等性后提交。普通接入只需知道：成功的管理写入会告诉你是否提交、当前修订、审计标识、缓存处理结果和警告。排查时可以看类似下面的 `MutationResult` 节选：
...
这是上例 `ruleResult` 一类管理写入的 envelope 节选；领域结果仍位于 `ruleResult.data`。它不是 `can()`、`init()` 或 `close()` 的返回结构。精确字段见对应 API 参考，不需要在理解生命周期时全部背下来。
...
就绪与事件处理见[生产运维](/zh/guide/production-operations)，精确公开证据面见[审计与健康 API](/zh/api/audit-and-health)。下一步理解规则怎样匹配请求，见[资源与规则](/zh/guide/resources-and-rules)。
