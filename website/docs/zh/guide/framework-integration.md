...
具体框架示例看 [Express 接入](/zh/examples/express) 和 [vext 接入](/zh/examples/vext)。

## 下一步看什么

- 想开始按步骤检查自己有没有漏项：看 [接入检查清单](/zh/guide/integration-checklist)
- 想直接看 Node.js 常见接法：看 [Express 接入](/zh/examples/express)
- 想看主运行时 API：看 [PermissionCore](/zh/api/permission-core)