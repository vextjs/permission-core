...
启用 `permission-core/menu` 后，还要独立选择菜单存储：

| Adapter | 适用场景 |
|---|---|
| `MemoryMenuStorageAdapter` | 测试与短生命周期示例 |
| `FileMenuStorageAdapter` | 本地、单进程菜单持久化 |
| `MonSQLizeMenuStorageAdapter` | 生产环境菜单、API binding、revision 与 audit 持久化 |

核心 adapter 和菜单 adapter 可以共享一个 MonSQLize 实例，但不能混用 collection，也不能同时拥有同一连接。推荐 core adapter 设置 `ownsConnection: true`，menu adapter 设置 `ownsConnection: false`，关闭时先 `menu.close()` 再 `pc.close()`。
...
- 接入模式决定你启用 `<METHOD>:<path>` 还是 `db:<collection>[:<field>]`；其中接口资源里的 `path` 指规范化后的命中路由路径
...
菜单存储还要保证 snapshot、revision 和 audit 一致。manifest 导入失败、关联授权保存失败或补偿失败时必须返回错误，不能留下“菜单已显示但接口授权未完成”的假成功状态。生产备份和迁移应同时覆盖核心权限 collection 与菜单 collection。
...
- [StorageAdapter API](/zh/api/storage-adapter)
- [MemoryAdapter API](/zh/api/memory-adapter)
- [FileAdapter API](/zh/api/file-adapter)
- [MonSQLizeStorageAdapter API](/zh/api/monsqlize-storage-adapter)

下一篇继续看 [权限缓存](/zh/guide/cache)。
