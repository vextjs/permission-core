...
如果你想直接看一段把 `roles.getRules()`、`roles.inspect()`、`getUserRoles()`、`setUserRoles()`、`getResources()` 串起来的后台代码，可继续看 [管理后台保存示例](/zh/examples/management-backend)。
...
如果你需要把错误结构固定成统一响应体，可以继续看 [错误处理与响应映射](/zh/guide/error-response-mapping)。

## 十、一个更贴近后台的落地顺序

如果你准备真正做后台页面，推荐顺序通常是：

1. 先确认资源字符串怎么写：看 [资源路径模型](/zh/guide/resource-paths)
2. 再确认角色和规则边界：看 [角色与规则](/zh/guide/roles-and-rules)
3. 再看角色与用户管理入口：看 [RoleManager](/zh/api/role-manager) 和 [UserRoleManager](/zh/api/user-roles)
4. 最后补错误响应和缓存失效：看 [错误处理与响应映射](/zh/guide/error-response-mapping) 和 [权限缓存](/zh/guide/cache)

## 下一步看什么

- 想先确认角色管理入口的具体方法：看 [RoleManager](/zh/api/role-manager)
- 想先确认用户角色绑定入口：看 [UserRoleManager](/zh/api/user-roles)
- 想直接看角色页和用户角色页怎么把管理 API 串起来：看 [管理后台保存示例](/zh/examples/management-backend)
- 想把后台错误结构返回给前端：看 [错误处理与响应映射](/zh/guide/error-response-mapping)
