...
想继续看运行时判断逻辑，下一篇看 [权限鉴权](/zh/guide/check-permission)。

## 下一步看什么

- 想继续理解运行时怎么判断权限：看 [权限鉴权](/zh/guide/check-permission)
- 想把角色页和用户角色页做成后台能力：看 [管理后台接入](/zh/guide/site-preview-release)
- 想按稳定顺序进入接入：看 [接入阅读顺序](/zh/guide/implementation-reading-order)
- 想直接看角色管理入口：看 [RoleManager](/zh/api/role-manager)