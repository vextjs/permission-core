...
具体错误处理可以结合 [Express 接入](/examples/express) 一起看。
...
- 想看更完整的错误处理和接口响应映射：看 [错误处理与响应映射](/guide/error-response-mapping)
- 想看错误码在真实接口里怎么处理：看 [Express 接入](/examples/express)
- 想回到运行时主入口：看 [PermissionCore](/api/permission-core)
- 想看角色和用户管理接口怎么落进后台：看 [管理后台接入](/guide/site-preview-release)