...
### Multiple APIs for one operation

Give related API bindings the same `permissionGroup` and choose `permissionMode: "any" | "all"` explicitly. Use `all` when every backend capability is required to complete the operation; use `any` only when the APIs are genuine alternatives. `strictApiBindings` disables sensitive operations whose required bindings are missing.
