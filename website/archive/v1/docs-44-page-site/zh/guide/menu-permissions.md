...
相关 API binding 使用同一个 `permissionGroup`，并明确选择 `permissionMode: "any" | "all"`。只有全部后端能力都具备才能完成操作时使用 `all`；多个 API 确实是替代路径时才使用 `any`。`strictApiBindings` 会在必需 binding 缺失时禁用敏感操作。
