...
function diagramFallbackText(id) {
    return {
        ...
        "role-menu-relationship": "The menu config is only the grantable resource inventory. The role authorization UI selects menus, views, load APIs, actions, and response fields from that inventory. preview only shows impact; grant, deny, or set writes role-menu authorization. After userRoles binds the role to a user, subject.menus projects that user's visible menu tree, action state, and returnable response fields.",
        "menu-config-lifecycle": "The admin side creates a menu config, menus, and views, then configures page load APIs, action APIs or UI permissions, and response fields. Role grants assign those capabilities, and backend guards protect APIs and response fields.",
...
function mermaidLabel(match, id) {
    const labels = {
        ...
        "role-menu-relationship": {
            "\"菜单配置<br/>资源目录\"": "\"Menu config<br/>resource inventory\"",
            "\"角色授权<br/>选择能力\"": "\"Role grant<br/>selects capabilities\"",
            "\"菜单 / 页面\"": "\"Menus / views\"",
            "\"加载接口\"": "\"Load APIs\"",
            "\"按钮 / 操作\"": "\"Actions\"",
            "\"响应字段\"": "\"Response fields\"",
            "\"preview<br/>只看影响\"": "\"preview<br/>shows impact\"",
            "\"grant / deny / set<br/>写角色授权\"": "\"grant / deny / set<br/>writes role grants\"",
            "\"userRoles<br/>绑定给用户\"": "\"userRoles<br/>binds user\"",
            "\"subject.menus<br/>运行时投影\"": "\"subject.menus<br/>runtime projection\"",
            "\"菜单树\"": "\"Menu tree\"",
            "\"按钮状态\"": "\"Action state\"",
            "\"响应字段裁剪\"": "\"Filtered response fields\"",
        },
        "menu-config-lifecycle": {
            "\"创建<br/>菜单配置\"": "\"Create<br/>menu config\"",
            "\"创建<br/>菜单\"": "\"Create<br/>menu\"",
            "\"创建页面<br/>orders-list\"": "\"Create view<br/>orders-list\"",
            "\"页面默认加载接口<br/>api:GET:/api/orders\"": "\"Page load API<br/>api:GET:/api/orders\"",
            "\"配置<br/>响应字段\"": "\"Configure<br/>response fields\"",
            "\"按钮权限\"": "\"Action permissions\"",
            "\"后端接口按钮<br/>api:*\"": "\"Backend action API<br/>api:*\"",
            "\"可选<br/>响应字段\"": "\"Optional<br/>response fields\"",
            "\"纯前端按钮<br/>ui:button:*\"": "\"Frontend-only<br/>ui:button:*\"",
            "\"角色授权\"": "\"Role authorization\"",
            "\"页面 / 接口<br/>按钮 / 字段\"": "\"Grant views / APIs<br/>actions / fields\"",
            "\"绑定角色<br/>给用户\"": "\"Bind role<br/>to user\"",
            "\"后端保护\"": "\"Backend protection\"",
            "\"assert / Vext<br/>守接口\"": "\"assert / Vext<br/>guards APIs\"",
            "\"filterResponse<br/>裁字段\"": "\"filterResponse<br/>filters fields\"",
        },
    }[id] ?? {};
